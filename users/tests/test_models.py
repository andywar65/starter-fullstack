...
class UserModelTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        profile.save()
        User.objects.create(username='nonames', password='P4s5W0r6',
            email='nonames@war.com')
        UserMessage.objects.create(user_id=user.uuid, subject='Foo',
            body='Bar')
...
class ProfileModelTest(TestCase):
    ...
    def setUpTestData(cls):
        print("\nTest profile models")
        # Set up non-modified objects used by all test methods
        user = User.objects.create(username='raw.ydna56', password='P4s5W0r6',
            email='ydna@raw.com')
        profile = user.profile
        img_path = Path(settings.STATIC_ROOT).joinpath('tests/image.jpg')
        with open(img_path, 'rb') as f:
            content = f.read()
        profile.temp_image = SimpleUploadedFile('image.jpg', content,
            'image/jpg')
        profile.save()
    ...
    def test_profile_fb_image(self):
        ...
        self.assertEquals(user.profile.fb_image.path,
            'uploads/images/users/image.jpg')
